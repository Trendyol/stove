...
internal interface CommonOps : RecordsAssertions {
  ...
  private fun consumerOffset(): Map<TopicPartition, Long> =
    adminClient.listConsumerGroups().all().get()
      .filterNot { it.groupId() == KafkaSystem.SUBSCRIBE_TO_ALL_GROUP_ID }
      .flatMap { group ->
  ...
  private suspend fun waitUntilLagsComputed(
    consumer: Consumer<String, String>,
    forTopic: String
  ): Collection<LagByTopic> {
    return { computeLag(consumerOffset(), producerOffset(consumer, consumerOffset())) }
      .waitUntilConditionMet(5.seconds, "computing lag") { it.topicPartition.topic() == forTopic }
  }
