...
| Exchange inspector ("Network tab") | 🚧 data feed ✅, rendering pending | The library side ships: both mocks implement `MockInteractionPublisher` and emit one `MockInteraction` per completed exchange — matched or not — with final status, latency, truncated bodies (message/byte counts for gRPC), near-miss candidates, trace id from `traceparent`, and **proven-only attribution** (`PROVEN_HEADER`/`PROVEN_BAGGAGE`/`PROVEN_STUB`/`UNATTRIBUTED` — never inferred; unattributed exchanges belong in a run-level lane). WireMock emits at `afterComplete` (timing final); gRPC via a call-observing interceptor that also surfaces client cancellations/deadlines the handler never sees. `DashboardSystem` forwards them as `MockInteractionEvent` (new proto oneof arm, wire-compatible). Bidi calls are now journaled too (matched and unmatched). Remaining: stove-cli rendering of the swimlane/network tab. |
| Unused-stub warnings | ✅ (library) | `UNUSED_STUB` warnings at test end from tagged-only evidence, both mocks. Suite-wide aggregation ("fixtures no test has needed for weeks") is CLI-side over the persisted events. |
