...
The reporter → stove-cli ingestion/storage/API path is now complete for mock interactions, warnings, and failure-time snapshots. What remains is SPA rendering of the per-test and unattributed run-level lanes, the derived C3 views, optional MCP queries over the persisted data, and the Theme E later bets.
...
The mock-facing dashboard work rides the same reporter → sqlite → dashboard (stove-cli) → MCP pipeline as the Kafka diagnostics theme. The reporter, sqlite persistence, live SSE payloads, and per-test/per-run HTTP reads are complete; SPA panels are next, while MCP exposure can follow independently.
...
| Cross-test match warnings | ✅ | Fixes M6 via the warnings pipeline: both mocks implement `MockWarningPublisher` and raise `CROSS_TEST_MATCH` at serve time when request and stub carry *provably different* test ids. Companion warnings from the same pipeline, computed at test end strictly from tagged-only evidence (`TestScopedJournal.taggedEntries`, so shared fixtures and non-propagating traffic can never accuse an unrelated test): `UNUSED_STUB` (registered by the test, never matched) and `UNVALIDATED_UNMATCHED` (the test's own unmatched requests with no `validate()` call). Forwarded as `MockWarningEvent`; never failures. |
...
| Exchange inspector ("Network tab") | 🚧 ingestion/storage/API ✅, SPA rendering pending | Both mocks emit one `MockInteraction` per completed exchange — matched or not — with final status, latency, truncated bodies (message/byte counts for gRPC), near-miss candidates, trace id from `traceparent`, and **proven-only attribution** (`PROVEN_HEADER`/`PROVEN_BAGGAGE`/`PROVEN_STUB`/`UNATTRIBUTED` — never inferred. `DashboardSystem` forwards the wire-compatible proto event; stove-cli now validates the run, preserves foreign/unknown test ids fail-open, publishes the live SSE payload, persists attributed and unattributed evidence in sqlite, and exposes per-test and per-run HTTP reads. Remaining: render the swimlane/network tab, including the unattributed run-level lane. |
| Unused-stub and mock warnings | 🚧 ingestion/storage/API ✅, SPA rendering pending | `UNUSED_STUB`, `CROSS_TEST_MATCH`, and `UNVALIDATED_UNMATCHED` warnings are emitted as diagnostics, never failures. Stove-cli now carries their optional test ids through unchanged, persists them, publishes live SSE payloads, and exposes per-test/per-run HTTP reads. Remaining: warning panels and suite-wide aggregation such as "fixtures no test has needed for weeks". |
