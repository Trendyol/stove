# Stove Portal in 0.23.0: See Your E2E Runs Live
...
As of **Stove 0.23.0**, you can use **Stove Portal** and the **`stove` CLI** to watch test execution in a local dashboard while tests are running.
...
### 3) Register Portal in Stove config
...
## How to Use Portal During Debugging
...
Use Portal as a local companion while iterating:
...
5. Use Reporting + Tracing in CI; use Portal primarily for local debugging speed

Portal is fault-tolerant by design. If CLI is not running, tests continue normally and Portal emission auto-degrades without breaking test execution.
...
You keep writing tests exactly as before; Portal captures entries/spans/snapshots automatically.

## Troubleshooting Quick Checks

- **UI stuck at waiting state:** ensure `stove` is running before tests.
- **No events appear:** verify `stove-portal` dependency and `portal {}` registration.
- **Port mismatch:** align `PortalSystemOptions(cliPort = ...)` with CLI `--grpc-port`.
- **Too much historical data:** run `stove --clear`.

## Links

- [Portal component docs](../Components/18-portal.md)
