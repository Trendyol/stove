...
### MCP server in the `stove` server

The server now exposes a local **Model Context Protocol** endpoint so AI agents can triage failed tests through compact, structured tools instead of loading entire logs into context.
...
Upgrade the `stove` server to 0.24.0:
