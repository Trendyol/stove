...
## Experimental WireMock DSL

WireMock gains an opt-in structured DSL that keeps request matching, response construction, and retry behaviour readable without exposing WireMock scenario state:
...
The DSL is marked with the general `ExperimentalStoveApi` opt-in annotation from `stove`, allowing future experimental APIs across modules to use one annotation.
