...
`rawStub(name?)` is the managed escape hatch for native WireMock mappings. It keeps Stove's test metadata, reporting, call journal, and cleanup, while direct `server()` access remains available for deliberately unmanaged operations.
