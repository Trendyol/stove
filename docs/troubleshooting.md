...
| Disk filling with old run data | Set `--retention-runs-per-app` (or `STOVE_RETENTION_RUNS_PER_APP`) to limit completed runs per app; use `stove --clear` for a full wipe |
