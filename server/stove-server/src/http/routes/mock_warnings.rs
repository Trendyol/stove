...
#[utoipa::path(
  ...
  responses((status = 200, description = "Mock diagnostics attributed to the test", body = Collection<MockWarning>))
)]
pub async fn get_test_mock_warnings(
  State(state): State<AppState>,
  Path((run_id, test_id)): Path<(String, String)>,
  Query(query): Query<PageQuery>,
) -> Result<Json<Collection<MockWarning>>, AppError> {
  if query.page {
    return Ok(Json(Collection::Page(
      state
        .repository
        .mock_warnings_page(run_id, Some(test_id), false, query)
        .await?,
    )));
  }
  ...
  Ok(Json(Collection::Legacy(
    state
      .repository
      .read_async(move |repository| repository.get_mock_warnings_for_test(&run_id, &test_id))
      .await?,
  )))
}
...
#[utoipa::path(
  ...
  responses((status = 200, description = "All mock diagnostics in the run", body = Collection<MockWarning>))
)]
pub async fn get_run_mock_warnings(
  ...
  if query.page {
    return Ok(Json(Collection::Page(
      state
        .repository
        .mock_warnings_page(run_id, None, false, query)
        .await?,
    )));
  }
  if query.cursor.is_some() || query.limit.is_some() || query.search.is_some() {
  ...
  Ok(Json(Collection::Legacy(
    state
      .repository
      .read_async(move |repository| repository.get_mock_warnings_for_run(&run_id))
      .await?,
  )))
}
...
#[utoipa::path(
  ...
  tag = "mocks",
  params(("run_id" = String, Path, description = "Run identifier"), PageQuery),
  responses((status = 200, description = "Mock diagnostics not attributed to a test", body = Collection<MockWarning>))
)]
pub async fn get_unattributed_run_mock_warnings(
  ...
  if query.page {
    return Ok(Json(Collection::Page(
      state
        .repository
        .mock_warnings_page(run_id, None, true, query)
        .await?,
    )));
  }
  if query.cursor.is_some() || query.limit.is_some() || query.search.is_some() {
  ...
  Ok(Json(Collection::Legacy(
    state
      .repository
      .read_async(move |repository| repository.get_unattributed_mock_warnings_for_run(&run_id))
      .await?,
  )))
}
