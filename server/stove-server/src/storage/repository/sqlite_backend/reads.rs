...
impl SqliteBackend {
  ...
  pub(in crate::storage::repository) fn runs_page(
    ...
    database.conn().transaction(|conn| {
      ...
      if let Some(app) = &request.app { query = query.filter(runs::app_name.eq(app)); }
      if let Some(cursor) = &request.cursor { query = query.filter(runs::started_at.lt(&cursor.started_at).or(runs::started_at.eq(&cursor.started_at).and(runs::id.lt(&cursor.id)))); }
      if !request.metadata.is_empty() { query = query.filter(diesel::dsl::sql::<diesel::sql_types::Bool>("NOT EXISTS (SELECT 1 FROM json_each(").bind::<Text, _>(serde_json::to_string(&request.metadata)?).sql(") wanted WHERE NOT EXISTS (SELECT 1 FROM json_each(runs.metadata) actual WHERE actual.key = wanted.key AND actual.value = wanted.value))")); }
      if !request.search.is_empty() { query = query.filter(diesel::dsl::sql::<diesel::sql_types::Bool>("LOWER(id || ' ' || app_name || ' ' || CAST(metadata AS TEXT)) LIKE ").bind::<Text, _>(&request.pattern).sql(" ESCAPE '\\'")); }
      let items = query.order((runs::started_at.desc(), runs::id.desc())).limit(i64::try_from(request.limit + 1).unwrap_or(1001)).select(runs::all_columns).load::<RunRow<String>>(conn)?.into_iter().map(Run::from).collect();
      request.finish(items, crate::storage::repository::distributed::live_event_id_to_u64(bounds.watermark)?)
    })
  }
  ...
  pub(in crate::storage::repository) fn tests_page(
    ...
    database.conn().transaction(|conn| {
      ...
      if !request.search.is_empty() {
        query = query.filter(
          diesel::dsl::sql::<diesel::sql_types::Bool>("LOWER(test_name || ' ' || spec_name) LIKE ")
  ...
  pub(in crate::storage::repository) fn entries_page(
    ...
    database.conn().transaction(|conn| {
      let bounds = diesel::sql_query(BOUNDS_SQL).get_result::<ReplayBounds>(conn)?;
      let sql = if request.kind == crate::storage::repository::pagination::EvidenceKind::RawEntries
      {
    ...
    database.conn().transaction(|conn| {
      ...
      let rows = query
        ...
        .select((
          ...
          diesel::dsl::sql::<diesel::sql_types::BigInt>("length(CAST(state_json AS BLOB))"),
        ))
  ...
  pub(in crate::storage::repository) fn snapshot_detail(
    ...
    Ok(
      snapshots::table
        ...
        .first::<SnapshotRow>(database.conn())
        .optional()?
        .map(Snapshot::from),
    )
  }
  ...
  pub(in crate::storage::repository) fn mock_interactions_page(
    ...
    database.conn().transaction(|conn| {
      let bounds = diesel::sql_query(BOUNDS_SQL).get_result::<ReplayBounds>(conn)?;
      let mut query = mock_interactions::table.filter(mock_interactions::run_id.eq(&request.run)).filter(mock_interactions::id.gt(request.after)).into_boxed::<diesel::sqlite::Sqlite>();
...
impl crate::ingest::PreparationLookup for SqliteConnection {
