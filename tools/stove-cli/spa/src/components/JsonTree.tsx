...
function JsonTreeNode({
  ...
  if (depth >= maxDepth) {
    return (
      <div className="leading-6 text-[var(--stove-text-muted)]" style={{ marginLeft: depth * 14 }}>
  ...
  const omittedChildren = children.length - visibleChildren.length;
      ...
      {effectiveExpanded && (
        <>
          {children.length === 0 ? (
            ...
            visibleChildren.map(([childLabel, childValue]) => (
              <JsonTreeNode
          ...
          {omittedChildren > 0 && (
            <div
              ...
              style={{ marginLeft: (depth + 1) * 14 }}
            >
              {omittedChildren} more {Array.isArray(value) ? "items" : "keys"} hidden
            </div>
          )}
          <div
