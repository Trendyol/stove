...
class KafkaSystem(
  ...
  suspend fun publish(
    ...
    serde: Option<StoveSerde<Any, Any>> = None,
    ...
    val record = ProducerRecord<String, Any>(
      ...
      partition.getOrNull(),
      key.getOrNull(),
      serde.getOrElse { context.options.serde }.serialize(message),
  ...
  @KafkaDsl
  suspend fun <T : Any> publishWithSerde(
    topic: String,
    message: T,
    serde: StoveSerde<Any, T>,
    key: Option<String> = None,
    partition: Option<Int> = None,
    headers: Map<String, String> = mapOf(),
    testCase: Option<String> = None
  ...
  override suspend fun beforeRun() {
    stoveKafkaSerdeRef = context.options.serde
  }
