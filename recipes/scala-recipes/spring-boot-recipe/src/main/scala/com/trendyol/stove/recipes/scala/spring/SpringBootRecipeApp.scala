...
object SpringBootRecipeApp {
  ...
  def run(args: Array[String], configure: SpringApplication => _): ConfigurableApplicationContext = {
...
class HelloWorldController(private val currentThreadRetriever: CurrentThreadRetriever) {
  @GetMapping
  def hello(): String = "Hello, World! from " + currentThreadRetriever.getCurrentThreadName
}
